...
export class HorseListCategoriesComponent implements OnInit, OnDestroy {
    ...
    getRangesIdToSearchAfter() {
        ...
        this.priceRanges.forEach(range => {
            ...
            if (rangeValues.length === 1 && (rangeValues[0] <= this.searchModel.PriceModel.MaxPrice || rangeValues[0] <= this.searchModel.PriceModel.MinPrice)) {
                rangeIds.push(range.Id);
            } else if (rangeValues[0] >= this.searchModel.PriceModel.MinPrice || rangeValues[1] <= this.searchModel.PriceModel.MaxPrice) {
                rangeIds.push(range.Id);
            }
        });
    ...
    getPriceRangeModelIntValues(priceRangeModelValue: string) {
        ...
        priceRangeValues.push(parseInt(values[0].replace(",", "")));
        priceRangeValues.push(parseInt(values[1].replace(",", "")));
